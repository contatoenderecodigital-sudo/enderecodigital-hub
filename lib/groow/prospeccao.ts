...
/**
 * Motor de prospeccao: busca no Google Places, pontua e filtra.
 *
 * Extraido da rota de admin para o painel do parceiro usar o MESMO codigo. A
 * alternativa era o parceiro chamar /api/admin por dentro, o que exigiria uma
 * porta dos fundos no middleware, ou duplicar a formula de score, que ia
 * divergir na primeira vez que alguem mexesse em um dos dois.
 */
...
export type ResultadoBusca =
  | { ok: true; empresas: unknown[]; query: string; totalBruto: number; foraDoRaio: number; centro: Ponto | null; raioKm: number | null; aviso: string | null }
  | { ok: false; status: number; error: string };

export async function buscarEmpresas(body: ParamsBusca): Promise<ResultadoBusca> {
  const key = process.env.GOOGLE_PLACES_API_KEY;
  if (!key) {
    return { ok: false, status: 500, error: "GOOGLE_PLACES_API_KEY não configurada no servidor." };
  }

  // params vem por argumento
  ...
  if (!nicho || (!cidade && !temCentro)) {
    return { ok: false, status: 400, error: "Informe o nicho e a cidade, ou marque um ponto no mapa." };
  }
  ...
  try {
    ...
    for (let i = 0; i < maxPaginas; i++) {
      ...
      if (!resp.ok) {
        ...
        if (i === 0) {
          return { ok: false, status: 502, error: `Google Places retornou erro ${resp.status}. Verifique se a chave está válida e a Places API (New) ativada.` };
        }
    ...
    const aviso =
      empresas.length === 0 && todas.length > 0
        ...
        : null;
    return { ok: true, empresas, query: textQuery, totalBruto: todas.length, foraDoRaio, centro, raioKm, aviso };
  } catch (err) {
    console.error("[prospeccao] error:", err);
    return { ok: false, status: 500, error: "Não foi possível buscar as empresas." };
  }
}
