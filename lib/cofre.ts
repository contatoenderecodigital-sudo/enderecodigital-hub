...
function chave(): Buffer {
  ...
  if (!s || s.length < 12) {
    throw new Error("SENHAS_CHAVE não configurada (qualquer frase longa serve; se trocar, as senhas já salvas não abrem mais).");
  }
  return createHash("sha256").update(s).digest();
}

export function temChaveCofre(): boolean {
  const s = process.env.SENHAS_CHAVE;
