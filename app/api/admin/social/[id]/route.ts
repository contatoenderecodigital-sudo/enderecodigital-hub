...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const ideias = await query<IdeiaRow>(`SELECT * FROM social_ideias WHERE id = ? LIMIT 1`, [Number(id)]);
    ...
    const conteudos = await query(
      `SELECT id, tipo, titulo, corpo, legenda, hashtags, status, custo_usd FROM social_conteudos WHERE ideia_id = ? ORDER BY id DESC LIMIT 1`,
      [Number(id)]
    );
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await garantirColuna("social_conteudos", "custo_usd", "DECIMAL(8,4) NULL DEFAULT NULL AFTER hashtags");
    const r = await exec(
      `INSERT INTO social_conteudos (ideia_id, tipo, titulo, corpo, legenda, hashtags, custo_usd) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [ideia.id, ideia.tipo, g.titulo.slice(0, 250), g.corpo, g.legenda, g.hashtags.slice(0, 490), g.custo_usd]
    );
    await exec(`UPDATE social_ideias SET status = 'gerada' WHERE id = ?`, [ideia.id]);
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (body.acao === "descartar") {
      await exec(`UPDATE social_ideias SET status = 'descartada' WHERE id = ?`, [Number(id)]);
      return NextResponse.json({ ok: true });
    }
    if (body.conteudo_id && body.status_conteudo && ["rascunho", "aprovado", "publicado"].includes(body.status_conteudo)) {
      await exec(`UPDATE social_conteudos SET status = ? WHERE id = ? AND ideia_id = ?`, [body.status_conteudo, Number(body.conteudo_id), Number(id)]);
      return NextResponse.json({ ok: true });
    }
